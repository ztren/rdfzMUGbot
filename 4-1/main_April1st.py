...
def returner(msg):
    ...
    S = ['123','卷卷','酒精灯','isxi','raber','若梦','FunctionSine','MUGBot']
    s = '[{0}]'.format(S[randint(0,len(S)-1)])
    f = ''
    ...
    if msg.type == 'Picture':
        ...
        if randint(1,15) == 1:
            if randint(1,3) == 1:
                ...
                if randint(1,3) == 1:
                    kk = ['我理论骨折光了','我制霸ARCAEA了！','我制霸Dynamix了！','我制霸epsilon了！','我理论病女了','我只能爬','我是你社最强群员','我和123贴贴']
                    k = kk[randint(0,len(kk)-1)]
                else:
                    k = msg.text
                group.send(MUGStr.rpt.format(k,tn))
    if (msg.text == '.jrrp') | (msg.text == '。jrrp'):
        ...
        try:
            x1,x2=query(s).split('###')
            x3 = '12.70'
            songlist = ['Grievous Lady','Fracture Ray','SAIKYO STRONGER','Burn','Stasis','iL','Longinus','ANiMA','Hemisphere','Saika','Aleph-0','もぺもぺ']
            x2 = 'Recent Play: \n' + songlist[randint(0,len(songlist)-1)] + '\n'+ x2
        ...
        if ' ' in msg.text[8:]:
            ...
            x = msg.text[8:].split('/')
        x += ['114514','生草','爬','膜raber','膜123','膜卷卷','膜isxi']
        if len(x) >= 2:
